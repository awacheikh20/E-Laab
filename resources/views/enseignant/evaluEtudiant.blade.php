...
@section('content')
    <div class="w-[95%] m-auto max-h-screen overflow-visible mt-20">
        <div class="flex justify-center items-center pb-5 ">
            <p class="text-blue-900 font-bold text-2xl italic">Bilan de l'évaluation : {{ $data['evaluation']['nom'] }}</p>
        </div>
            ...
            <form action="" method="post">
                ...
                <div class="w-full bg-white">
                    <span class="text-blue-900 text-lg flex justify-center items-center">
                        ...
                        <input readonly name="nom" type="text" value="{{ $data['evaluation']['nom'] }}" placeholder="Entrez le libellé de l'évaluation"><br>
                    </span>
                </div>

                <div id="questions">
                    @foreach ($data['evaluation']['questions'] as $index => $question)
                        <div class="question shadow-md shadow-blue-900/400 hover:shadow-blue-900 rounded-b-2xl w-full mb-5">
                            ...
                            @foreach ($question['propositions'] as $slug => $proposition)
                                <div class="propositions p-1">
                                    <input class="ml-16" type="checkbox" name="corrections[{{ $index }}][]" id="" value="{{ $proposition }}" @if(in_array($proposition, $question['reponses_etudiant'])) checked @endif>
                                    <span class="ml-16">{{ $slug+1 }}.</span><input readonly class="ml-2 w-[80%]" type="text" value="{{ $proposition }}" name="propositions[0][]" placeholder="Entrez une proposition" @if(in_array($proposition, $question['reponses_etudiant'])) style="color: green;" @else style="color: red;" @endif><br><br>
                                </div>
                            @endforeach
                        </div>
                    @endforeach
